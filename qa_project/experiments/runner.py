...
def run_all_conditions(df: pd.DataFrame,
                       ...
                       conditions_to_run: list[str] | None = None) -> dict[str, pd.DataFrame]:
    """
    Run experimental conditions on the supplied DataFrame.

    Args:
        df: DataFrame with questions
        llm: LLM interface instance
        retriever: Retriever instance
        conditions_to_run: List of condition names to run. If None, runs all.
                          Options: ['closed_book', 'rag_only', 'cot_only', 
    ...
    # If no specific conditions requested, run all
    if conditions_to_run is None:
        conditions_to_run = ['closed_book', 'rag_only', 'cot_only', 
                           'rag_cot', 'rag_cot_selfverify']

    results = {}

    log_header(f"Running Experimental Conditions ({len(conditions_to_run)} selected)")

    # Map condition names to their runner functions
    condition_runners = {
        'closed_book': lambda: _run_closed_book(df, llm),
        'rag_only': lambda: _run_rag_only(df, llm, retriever),
        'cot_only': lambda: _run_cot_only(df, llm),
        'rag_cot': lambda: _run_rag_cot(df, llm, retriever),
        'rag_cot_selfverify': lambda: _run_rag_cot_selfverify(df, llm, retriever),
    }

    # Run only the requested conditions
    for condition in conditions_to_run:
        if condition in condition_runners:
            results[condition] = condition_runners[condition]()
        else:
            log_info(f"Warning: Unknown condition '{condition}', skipping")
